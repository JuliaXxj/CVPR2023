...
class LinearRegionCount(object):
    ...
    @torch.no_grad()
    def calc_LR(self):
        res0 = torch.matmul(self.activations.half(), (1-self.activations).T.half()) # each element in res: A * (1 - B)

        res = res0 + res0.T # make symmetric, each element in res: A * (1 - B) + (1 - A) * B, a non-zero element indicate a pair of two different linear regions
